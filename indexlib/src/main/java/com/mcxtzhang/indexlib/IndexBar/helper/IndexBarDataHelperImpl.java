    ...
    public IIndexBarDataHelper sortSourceDatas(List<? extends BaseIndexPinyinBean> datas) {
        ...
        Collections.sort(datas, new Comparator<BaseIndexPinyinBean>() {
            ...
            public int compare(BaseIndexPinyinBean lhs, BaseIndexPinyinBean rhs) {
                ...
                String lhspy = lhs.getBaseIndexPinyin();
                String rhspy = rhs.getBaseIndexPinyin();

                if (TextUtils.isEmpty(lhspy) || TextUtils.isEmpty(rhspy)) {
                    return 1;
                }
                ...
                if (!lhs.isNeedToPinyin()) {
                    ...
                    return 0;
                } else if (lhs.getBaseIndexTag() != null && lhs.getBaseIndexTag().equals("#")) {
                    return -1;
                } else if (rhs.getBaseIndexTag() != null && rhs.getBaseIndexTag().equals("#")) {
                    return 1;
                } else {
                    try {
                        Log.d("compare","rhspy.compareTo(lhspy) = " +rhspy.compareTo(lhspy));
                        if (rhspy.compareTo(lhspy) == 0) {
                            return 0;
                        } else if (rhspy.compareTo(lhspy) < 0) {
                            return 1;
                        } else {
                            return -1;
                        }
