...
public class PhotoUpAlbumHelper extends AsyncTask<Object, Object, Object>{
	...
	void buildImagesBucketList() {
		...
		if (cur.moveToFirst()) {
			 ...
			 */
			do {
				int start=cur.getString(photoPathIndex).lastIndexOf("/")+1;
				int end=cur.getString(photoPathIndex).lastIndexOf(".");
				if (end>start){
					if(cur.getString(photoPathIndex).substring(start,end)
							.replaceAll(" ", "").length()<=0){
